...
- **敵の可愛さ（2026-09-07 ユーザー「ホロウナイトくらいの可愛さ」）**: 丸い体・大きな頭・大きくて単純な目（小さなハイライト）・小さな手足・少し寂しげで愛嬌のある表情。怖さは表情でなく「形の異様さ」で出す（牙をむく・睨むは禁止語＝negative に scary/menacing/sharp teeth/angry）。プロンプトの土台は `docs/pixellab/enemies-cute.json` の `style`。
