...
**舞台＝HD-2D ジオラマ（2026-09-07 「オクトラ風なら」）**: 舞台だけ本物の3D、キャラは2Dドット。3Dの箱庭（地面・段々の台地・崖・遺跡の柱・板の木と茂み・ランタン）に透視カメラと被写界深度（焦点はキャラの立つ線）、月光の影、霧、ブルーム、粒子。キャラの板は焦点面に立つので「1ドット＝画面4px」のまま。紙の UI（Overlay キャンバス）には何も掛からない。
...
| 木・茂み・岩（板） | 未接続（次の段） | 32×48 / 24×16 / 24×14 | 被写界深度でぼけるので整数倍は不要 |
