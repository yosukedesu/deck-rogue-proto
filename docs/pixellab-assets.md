...
| 種別 | フォルダ | 実寸 (px) | 枚数 | 備考 |
|---|---|---|---|---|
| 敵（通常） | enemies | **64×64**（3倍で192px） | 63 | 2026-09-07 改定: アニメ生成の寸法（64/128）に合わせて 96→64。待機は `<id>_idle.png` のシート（後で対応）。被弾は白フラッシュで代用 |
| 敵（エリート） | enemies | 128×128（2倍で256px） | 13 | 同上 |
| 敵（幕ボス） | enemies | 128×128（2倍。立ち位置と演出で格を出す） | 8 | 2026-09-07 改定: 160→128。第2形態は色替えで代用 |
| リーダー（戦闘内ちび） | leaders | **64×64**（3倍で192px。2026-09-07 このは実測OK） | 15 | 頭身低め（確定済みルール表「絵柄の頭身」）。立ち絵は人間の絵師へ外注 |
...
- **寸法はこの表のとおり**（96/128/160 のキャンバス、アイコンは 16/24/32）。画面には整数倍で置くので、余白を切らずキャンバスいっぱいに使わない（敵は下端を地面に接する、左右に 8〜12px の余白）。
- **向き**: リーダーは右向き、敵は左向き（正面向きでも成立する。左右反転はコード側でできる）。
- **頭身**: 戦闘内ちびは2〜2.5頭身のゆるかわ。塔の深層（幕3）ほど不穏に。
- **線と色**: 1px の暗い輪郭（墨 #3b2f2f 寄り）、面は3〜4階調。紙の UI（#f4ecd6）の上に貼り絵の縁が付くので、輪郭の外側に余計な発光を描かない。
- **アイコン**: 墨1色の線＋役割の色1つ（攻撃=薔薇 #d97b7b・ブロック=空 #7fa7c9・成長=苔 #8fae7b・勢い=蜂蜜 #e0b25a・呪文=藤 #a98cc4・リアクション=青緑 #7ab8b0）。背景透過。
- **待機の2コマ目**（`<id>_2.png`）は後回し。まず1コマで並べて相性を見る。
- **書き出し**は PNG・透過・実寸（拡大しない）。ファイル名は上の id に揃える（`Assets/Resources/Art/<種別>/` に置くだけで差し替わる。9スライスは名前で判定）。
