...
- **敵の絵柄＝ホロウナイトそのもの（2026-09-08 ユーザー「もっとホロウナイトに似せて。カラフルじゃなく寂しげな良さ」。前日の「可愛さ」の解釈〔丸くてカラフルなマスコット〕は却下）**: 墨黒・骨白・くすんだ青灰だけの低彩度（アクセントは薄い青緑1色まで）、顔は滑らかな白い仮面に空洞の黒い目、口は無いか小さい、細い手足、静かで寂しげ。笑顔・頬の赤み・鮮やかな色は negative へ。虫のような存在として言い換える（フードやローブだけだと人間の子どもに描かれる）。プロンプトの土台は `docs/pixellab/enemies-hollow.json` の `style`。
