...
| 種別 | フォルダ | 実寸 (px) | 枚数 | 備考 |
|---|---|---|---|---|
| 敵（通常） | enemies | **128×128**（2倍で256px） | 63 | 2026-09-07 ユーザー裁定「128」: 描き込み優先。API のアニメ（スケルトン式）も 128 対応。待機は `<id>_idle.png` のシート（後で対応）。被弾は白フラッシュで代用 |
| 敵（エリート） | enemies | 128×128（2倍で256px。格は造形と演出で） | 13 | 同上 |
| 敵（幕ボス） | enemies | 128×128（**3倍で384px**） | 8 | 2026-09-07 改定: 160→128。表示倍率だけ3倍で格を出す。第2形態は色替えで代用 |
| リーダー（戦闘内ちび） | leaders | **128×128**（2倍で256px。2026-09-07 ユーザー裁定「128」） | 15 | 頭身低め（確定済みルール表「絵柄の頭身」）。立ち絵は人間の絵師へ外注 |
...
- **寸法はこの表のとおり**（人物と魔物は全て 128×128、アイコンは 16/24/32）。画面には整数倍で置くので、キャンバスいっぱいに使わず、敵は下端を地面に接し左右に 6〜10px の余白。
