...
**舞台（2026-09-07 段階1）**: 背景は UI と別のカメラに描く「舞台」で、ブルーム（月・蛍が光る）・ビネット・幕ごとの色調整・粒子（蛍 4/s・ほこり 8/s・舞う葉 0.7/s）・霧の揺れが掛かる。紙の UI（Overlay キャンバス）には掛からない。
背景素材 `Art/bg/act<N>.png` を置けば水彩の空の代わりにそのまま舞台に敷かれる（1920×1080 想定。PixelLab なら 480×270 を4倍）。段階2＝遠景・中景・近景の3層パララックス（遠景は事前にぼかす）、段階3＝2Dライトは素材が揃ってから。
