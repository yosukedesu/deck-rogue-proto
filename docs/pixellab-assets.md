...
| コスト玉 | ui | 26×26 | 1 | `cost_orb.png`（済。2倍で 52 ＝ カードの玉。数字は文字で乗せる）。**2026-09-16 カラーテーマ「黒鉄と真鍮」でエナジー＝脈の青緑に。いまの png は真鍮の玉を色相だけ青緑へ回した派生（陰影は元のまま。原本は `docs/pixellab/ui/cost_orb_brass.png`）＝PixelLab で青緑のマナの玉を描き直したら差し替える（残高が要る）** |
