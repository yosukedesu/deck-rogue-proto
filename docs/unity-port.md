...
- 追補（同日 ユーザー「強化後はゲーム中いつでも確認できるカードの基本機能。本家の仕様をよく確認すること」）: 「鍛えると→」の1行は撤回し、本家の形に。①`CardPopup`＝どの画面の札も長押し 0.5秒（`LongPressOpen`: 押す→動かさず 0.5秒。ドラッグ開始で取り消し。開いた直後の離しはクリック扱いにしない `SuppressClicksUntil`）か右クリック（手札を除く）で 1.6倍の札＋「鍛えた後を見る」切り替え＋用語の説明。`FxLayer` に置くので `Rebuild` で消える（切り替えは中で描き直すだけ）。②焚き火・ショップの鍛える画面は本家 Smith の「元 → 鍛えた後」（`ForgePreviewArea`/`ShowForgePair`/`AttachForgePreview`＝cell のホバーとタップ）。グリッドの札は raycast を受ける（`interactable=true`）ようにし、押す・離す・右クリックは cell 側で受ける（子の札に付けると PointerExit を横取りしてツールチップが消えなくなる）。自動操縦に battle-popup と鍛える画面の並び（`PreviewFirst`）を追加。
- 追補（同日 ユーザー「若幹の一撃が一回しか攻撃していない。表記は×2」）: 原因は表示層。`CardText.ConditionLabel` に `minEnergyMax` と `actionKinds` が無く、条件つきの2発目が無条件の行と同じ文になって `Collapse` が「ダメージ6 ×2回」に畳んでいた。データ全5色の効果フィールドを走査して同じ穴を確認: 条件キーで抜けていたのはこの2種だけ（該当札＝若幹の一撃・大地の唸り〔エナジー上限5以上〕・共鳴する茨〔敵の行動が筋力上げ・応援の時〕）。効果側の他のフィールド（target・pierce・xHits・growthMultiplier・momentumMultiplier・spendBlock・exhaustThreshold・summonId・amountMax）は表示済み。engine は正しく1発だった。
- 拡大表示の層: `FxLayer` は CanvasGroup で raycast を通さないため、入力を受ける `GameRoot.PopupLayer` を別に作り、開くたびに最後尾へ回す（戦闘の層が後から作られても上）。`Rebuild` の画面差し替え時に閉じる。
