...
| 4 | map-structure | ショップ員数の固定化（重み5%→固定3/幕） | ✅ ddf838b ショップ固定3+到達保証 |
| 5 | map-structure | 部屋タイプ員数の幕別テーブル化（?のガウス分布・幕2/3で-1） | 🔶 ?=22%は本家一致の物差し |
| 6 | map-structure | ボス前3行の焚き火禁止（休憩の上限制約） | ✅ dc17ea9 ボス前3行の焚き火禁止 |
| 7 | map-structure | ボス複数種+未撃破優先ローテ（BossDiscoveryOrder） | 📌 roadmap「本家形フェーズ2」: ボス複数種+未撃破優先 |
| 8 | map-structure | 初回ラン限定のチュートリアル導線（DiscoveryOrder上書き） | 📌 roadmap「本家形フェーズ2」: チュートリアル導線 |
| 9 | map-structure | イベント専用エンカウンター（?から入る特別報酬付き戦闘） | 📋 M |
| 10 | map-structure | 選択にならない分岐の枝刈り（PruneAndRepair相当） | 📋 M |
| 11 | map-structure | エリート下限行の本家寄せ（行2→行5以降）の再裁定 | ✅ ddf838b 幕1エリート下限行4 |
| 12 | map-structure | 構造つまみの器（高難度段でエリート増・宝箱行→エリート行） | 📌 roadmap「本家形フェーズ2」: 難易度つまみ系(凍結) |
| 13 | map-structure | 幕1の2バイオーム択（Overgrowth/Underdocks方式） | 📌 roadmap「本家形フェーズ2」: 2バイオーム |
...
| 19 | behavior-grammar | Queen式SetMoveImmediate＝仲間死亡の瞬間に宣言済み意図を強制差し替え | ❌ 見送り: 宣言時固定則を破る。転職は次宣言から、で足りる |
| 20 | behavior-grammar | EncounterMemberのprelude上書き＝スロット役割分化（Exoskeleton式） | 📋 S |
| 21 | behavior-grammar | 技の恒久成長（growPerUse/growHitsPerUse）＝使うたび育つ技で「戻らない恐怖」を作る | ✅ 606304b 技の恒久成長(狩人・巨面) |
| 22 | behavior-grammar | 敵の召喚行動（kind:'summon'）＝戦闘中に味方を補充する敵 | 📋 L |
| 23 | behavior-grammar | 眠りの被ダメ覚醒（wakeOnDamage）＝眠れる鉄卵の「起こす前に削るか」を本物の二択にする | ✅ 606304b 被弾覚醒(鉄卵) |
| 24 | behavior-grammar | 敵のアーティファクト（デバフ無効チャージ）＝延焼・急所・威圧・混乱への構造的な問い | ✅ 606304b 敵アーティファクト(延焼は弾かない) |
...
| 39 | debuff-system | 敵アーティファクト（デバフ付与をN回弾く）＝デバフ依存デッキへの問い | ✅ 606304b 同件 |
...
| 44 | debuff-system | 錯乱デバフ（Confused/Snecko式）＝ドローごとにコストを0〜3へ乱数化するボス級の霧 | ❌ 見送り: ボス級の霧。今は不要 |
| 45 | debuff-system | 1ターン被ダメ累計上限（HardenedShell式）＝装甲の相補変種・バースト対策 | ✅ 606304b ターン装甲(第3波) |
...
| 55 | gimmick-variants | カード盗み — デッキの人質（盗んだ敵は必ず逃げる・倒せば返る） | ❌ 見送り: 体験が過激(デッキの人質)。金盗みの器で十分 |
...
| 60 | gimmick-variants | 眠りの被弾覚醒 — 鉄卵に「傷つけると目を覚ます」を追加 | ✅ 606304b 同件 |
...
| 63 | gimmick-variants | 成長吸い — プレイヤーの成長を奪い、倒せば返す（憑依型） | ❌ 見送り: 緑への実質ナーフ成分 |
| 64 | gimmick-variants | ターン装甲（硬化した殻）— 1ターンの被ダメ累計キャップ（バーストへの問い） | ✅ 606304b ターン装甲(第3波・同件) |
...
| 69 | enemy-numbers | 幕2に帯上端ソロを新設（HunterKiller/LouseProgenitor枠・実効HP120〜135） | ✅ 528832b 大顎の狩人(幕2重量級) |
| 70 | enemy-numbers | 幕3に帯上端ソロを新設（SlimedBerserker枠・実効HP230前後・札汚染で殴る大物） | ✅ 528832b 汚泥の大暴れ(幕3札汚染の大物) |
...
| 78 | enemy-numbers | 帯設計方針の明文化——「幕1増強はHP・幕2/3増強は上端個体・打点はこれ以上触らない」 | ✅ CLAUDE.md 本家形パッケージ行に明文化 |
| 79 | enemy-numbers | ボス随伴（KinPriest型: 本体+子×2）の導入検討 | 📌 roadmap「本家形フェーズ2」: ボス随伴 |
...
| 94 | fairness-intent-ui | 【裁定要】実値表示モードの標準トグル化（退屈診断④の判定実験） | ✅ 実値表示トグル(デバッグ枠。仕様不変・ジャーナル記録) |
| 95 | fairness-intent-ui | 【裁定要】不明意図（❓Hidden）の限定導入——「どちらの技か伏せる」敵 | ❌ 見送り: 予告原則と衝突 |
...
| 99 | economy-events | カードを盗む敵（Swipe式・倒せば特別報酬で返る） | ❌ 見送り: 同件 |
...
| 103 | economy-events | 幕頭の祝福（Ancient/Neow式の幕遷移ブーン選択） | 📌 roadmap「本家形フェーズ2」: 幕頭の祝福 |
| 104 | economy-events | 消耗品（ポーション）システムの最小導入 | 📌 roadmap「本家形フェーズ2」: ポーション |
| 105 | economy-events | 呪い札の性質ラダー（Doubt/Shame/Regret式） | 📋 S |
| 106 | economy-events | 継続窃盗の敵（Thievery式・毎ターン少額ドレイン） | 🔶 継続窃盗 |
| 107 | economy-events | ショップ員数の固定保証（本家=固定3/幕） | ✅ ddf838b 同件 |
...
| 111 | design-guardrails | アーキ別勝率の分散を一級指標化（sim要約行＋構造詰みセルのwhitelistテスト） | ✅ variance.slow.test.ts+baseline(0%セル93の台帳) |
| 112 | design-guardrails | 敗北の感触を2分類する検証プロトコル（評価バーに「構築の失敗/理不尽」・LLMラン定型質問） | ✅ 敗北の感触2分類(評価バー+レポート敗因列) |
...
| 115 | design-guardrails | 戦闘評価テレメトリの横断集計スクリプト（敵×評価の平均表＝作り直し候補の機械抽出） | ✅ scripts/aggregate-ratings.ts (npm run ratings) |
| 116 | design-guardrails | 幕1序盤のWeak帯プール分離（StS2 GrabBagの簡易版＝初見体験の構造保証） | ✅ dc17ea9 Weak帯として実装済み |
| 117 | design-guardrails | 初回ラン限定の教師順（チュートリアル導線）はロードマップ項目として記録に留める | 📌 roadmap「本家形フェーズ2」: 同件 |
...
| 120 | design-guardrails | 予告（フェアネス）表示の網羅性テスト＋ラベル表の純モジュール化 | ✅ 表示網羅テスト(display-coverage.test+traits.ts+keywordHelp.ts) |
...
| 124 | design-guardrails | 幅あり意図のAB計測プロトコル（蜃気楼の面を実験器に退屈診断④の判定材料を取る） | ✅ 同件(人間ランのAB計測器) |
...
| 133 | critic | 【裁定要】リソース完全ゼロ化の1ターン劇薬（NoEnergyGain/NoDraw式） | ❌ 見送り: 霞み・拘束で足りる。完全ゼロ化は劇薬 |
| 134 | critic | 【裁定要】ステータス札を弾薬に変える対抗策カード（FlakCannon/Compact式） | 🔶 プレイヤーカード追加=凍結中 |
| 135 | critic | 【裁定要】アセンション式・経済税の器（数値でなく経済で締める難易度列） | 📌 roadmap「本家形フェーズ2」: 同件(凍結) |

## 追補（2026-09-02 ユーザー指摘）: 1戦闘の平均敵数

ミニングが拾い損ねていた最大の差。本家は幕1通常2.00体・3体以上が各幕2〜3・弱枠にも群れ。うちは幕1 1.38・Weak帯全ソロ・3体編成ゼロ。
新敵「小泥」＋編成11で 幕1 1.94／幕2 1.71／幕3 1.78 へ（map.test で幕別下限を機械固定）。「小さい体でHPを分散」は幕1巡航へのHPを盛らない本家側の答え。

## 追補（2026-09-02 ユーザー指示「本家並みの敵バリエーション・本家同等に」）

種類数を本家1バイオーム（≈15〜20種/幕）に合わせ、本家の「問いの型」を移植: 幕1+8・幕2+2・幕3+6・ボス+3（各幕2種を抽選）・編成+12。
幕1通常種10→18／幕2 21→24／幕3 15→22／ボス3→6。分裂体・孵化体のHP倍率継承を修正（残機ボスの前提）。
...
- ✅ 実装済み: 72件（バグ修正2件を含む。2026-09-02 残件議論で+19）
- 🔶 裁定待ち: 10件
- 📋 バックログ: 37件（形の問い系=幕1教師枠向き・汚染系・イベント戦・表示S。実装スケッチは各提案に保存）
- ❌ 見送り: 8件（理由は各行）
- 📌 roadmap送り: 9件（本家形フェーズ2）
