...
## 手で複製した表とゴールデンの穴（2026-09-14 人間ラン#10 で発見）
...
- 人間/Opus のジャーナル付きセーブ（ブラウザ/CLI = TS 製）は `dump-goldens.ts --from <save.json>` で3幕通しのゴールデンにできる（最も濃い照合）。
  Unity 製のセーブは C# 側の出力なので、TS の `replayStates` で再生して分岐点を探す道具になる（scratchpad の divergence スクリプト）。
