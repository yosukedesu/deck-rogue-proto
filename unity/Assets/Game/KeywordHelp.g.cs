// KeywordHelp.g.cs — scripts/gen-keyword-help.ts が src/ui/keywordHelp.ts から生成。手で編集しない。
...
namespace DeckRogue.Game
{
    public static class KeywordHelp
    {
        public static readonly Dictionary<string, string> Terms = new Dictionary<string, string>
        {
            ...
            { "虚弱", "カードのプレイで得るブロックが25%減る（切り捨て・最低1）。氷壁・伏せ札・置物由来は減らない。自分のターン終了時に1減る" },
            ...
            { "拘束", "残りNターンの間、1ターンにプレイできるカードは3枚まで。伏せる・発動するのは制限されない" },
            ...
            { "潜伏", "殻（ブロック）が尽きるまでHPにダメージが通らない。超過ぶんは捨てられる。貫通も殻に吸われ、粉砕は殻を割る。殻が割れると次の行動が噛みつきに変わる（割ったターンのうちに倒せば来ない）＝割るタイミングの問い" },
            { "因縁", "奇数ターン（1・3・5…）は無形で、1ヒットのHP損失が1に固定される。偶数ターンに実体化。延焼は通る。殴るターンと備えるターンのリズム" },
            { "バランス崩し", "攻撃を完全に防ぐ（HP損失0）と体勢を崩し、次の行動が隙になる。被攻撃前の軽減の伏せ札やブロックの積み増しに直接の報酬＝「あと何点で完全に防げるか」の計算" },
            { "眠り", "眠っている間は殻を積む。累計ダメージがしきい値に達すると目を覚まし、眠りをやめて本気になる。寝ている間に削るか、放置して準備するか" },
            ...
            { "被攻撃前", "敵の攻撃でダメージを受ける直前に動かせる（軽減向き）" },
            { "被攻撃後", "敵の攻撃でダメージを受けた後に動かせる（返し向き）" },
            { "敵強化時", "敵が筋力を上げた直後に動かせる" },
            { "敵防御時", "敵がブロックを得た直後に動かせる" },
            { "敵行動時", "敵の行動が確定した直後（実行前）に動かせる。打ち消し向き" },
            ...
            { "伏せ破壊", "足元の伏せ場を壊し、伏せた札を捨て札に送る。壊されるのは発動できる伏せ札（前のターン以前に伏せた札）だけで、今ターン伏せた準備中の札は見つからない。壊しつつ殴る敵は、壊してから攻撃する＝壊された罠は鳴らない" },
            { "伏せ場", "コストを先に払ってカードを伏せておく場所（基本1枠）。敵の行動の実値を見てから発動するか温存するかを選ぶ。壊されると伏せた札は捨て札へ。回収はできない" },
            { "伏せる", "コストを払って手札の1枚を伏せ場に置く＝罠を仕込む。伏せたターンは発動しない（準備）。翌ターンと翌々ターンの敵フェーズだけ発動でき、その2回で鳴らなければ期限切れで捨て札に戻る（消滅持ちは消滅）。敵は伏せ札を見ない（罠壊し・道化の破壊だけ例外）" },
            { "氷壁", "このブロックはターン開始で消えず持ち越される。通常ブロックを使い切った後に消費される" },
            { "詠唱数", "このターンにプレイしたカードの枚数（そのカード自身は数えない）。ターン開始でリセット" },
            { "霊気", "妨害や伏せ札の成功で溜まるエネルギー（戦闘中持続）。霊気放出で一気に叩きつける" },
            { "延焼", "敵に蓄積する継続ダメージ。毎敵ターン開始時に延焼値ぶんのダメージ（ブロック無視）を与えて1減る" },
            { "衝動", "山札の上からめくった「このターン限り」の手札。使わずにターンを終えると消滅する" },
            { "粉砕", "敵のブロックを全て破壊する（無視ではなく叩き割る）" },
        };

        /// <summary>文中に含まれる用語を長い順に探す (「呪いの烙印」が「烙印」より先に当たるように)</summary>
        public static List<string> FindIn(string text)
        {
            var found = new List<string>();
            if (string.IsNullOrEmpty(text)) return found;
            foreach (var k in Keys) if (text.Contains(k) && !found.Contains(k)) found.Add(k);
            return found;
        }
        ...
        static List<string> Keys
        {
            get
            {
                if (_keys == null) { _keys = new List<string>(Terms.Keys); _keys.Sort((a, b) => b.Length.CompareTo(a.Length)); }
                return _keys;
            }
        }
    }
