...
namespace DeckRogue.Game
{
    public static class KeywordHelp
    {
        public static readonly Dictionary<string, string> Terms = new Dictionary<string, string>
        {
            ...
            { "虚弱", "カードのプレイで得るブロックが25%減る（切り捨て・最低1）。氷壁・仕込み札・置物由来は減らない。自分のターン終了時に1減る" },
            ...
            { "拘束", "残りNターンの間、1ターンにプレイできるカードは3枚まで。仕込む・からくりを動かすのは制限されない" },
            ...
            { "バランス崩し", "攻撃を完全に防ぐ（HP損失0）と体勢を崩し、次の行動が隙になる。被攻撃前の軽減の仕込み札やブロックの積み増しに直接の報酬＝「あと何点で完全に防げるか」の計算" },
            ...
            { "打ち消す", "敵の行動1回を完全に無効化する（攻撃・防御・からくり壊し・筋力上げすべて対象）" },
            ...
            { "からくり壊し", "足元のからくりを壊し、仕込んだ札を捨て札に送る" },
            { "からくり", "一手を仕込んで足元に据える小さな匣。ぜんまいを巻いておき、敵の実値を見てから動かすか巻いたまま待つかを選ぶ。壊されると仕込んだ札は捨て札へ" },
            { "仕込む", "コストを払って手札の1枚をからくりに入れる。巻いたばかりの歯車は鳴るので、そのターンは魔物が気づいて動きを変えることがある（一巡すれば鳴りやむ）" },
            { "氷壁", "このブロックはターン開始で消えず持ち越される。通常ブロックを使い切った後に消費される" },
            { "詠唱数", "このターンにプレイしたカードの枚数（そのカード自身は数えない）。ターン開始でリセット" },
            { "霊気", "妨害や仕込み札の成功で溜まるエネルギー（戦闘中持続）。霊気放出で一気に叩きつける" },
