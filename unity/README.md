...
scripts/unity-win.sh shots [tour|battle|run] [seed]  # プレイヤーを自動操縦 (Assets/Game/Autopilot.cs) で起動し、各画面の PNG を unity/Shots/ に回収 (git 管理外)。battle はシード6が全状態を踏む。run は戦闘を自動で勝ちながらタイトル/マップ/報酬/?/店/レリック/焚き火/工房/敗北を撮る
