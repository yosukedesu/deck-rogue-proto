...
const TRIGGER_LABEL: Record<CardDef['effects'][number]['trigger'], string> = {
  ...
  onSetDestroyed: 'この伏せ場が壊された時・期限切れの時: ',
...
const CATALYST_LINE: Record<NonNullable<CardDef['fusionCatalyst']>, string> = {
  ...
  aoe: '⚗️ 触媒: 工房の素材にすると、結果の単体ダメージが敵全体になる',
}
              ...
              {s.pendingWindow?.stage === 'post' && (
                <div className="hint" style={{ marginBottom: 8 }}>
                  {effectiveIntent(s, s.pendingWindow.enemyIndex)?.kind === 'attack'
                    ? '※この攻撃はすでに解決済み——動かしても今回の被弾は取り消せない（返し・回復のための窓）'
  ...
  return (
    <div className="viewer-overlay" onClick={onClose}>
      <div className="viewer-panel" onClick={(e) => e.stopPropagation()}>
        ...
        {sorted === true && (
          <p className="hint">※山札は引き順が分からないよう並び替えて表示（本家準拠）</p>
        )}
