...
import { createRunInBattle, withHand } from './test-helpers.ts'
...
describe('ギルドリーダーのロースター', () => {
  ...
  it('ギルドランは2色で始まり、初期デッキは両色混成の10枚', () => {
    const run = createRunInBattle(7, 'set-confirm', 'leader_rakdos')
...
describe('ギルドパッシブ', () => {
  ...
  it('わかば (セレズニア): 置物が場に出るたび成長+1', () => {
    const run = createRunInBattle(3, 'set-confirm', 'leader_selesnya')
  ...
  it('くろは (ゴルガリ): カードが消滅するたび成長+1', () => {
    const run = createRunInBattle(3, 'set-confirm', 'leader_golgari')
  ...
  it('あさひ (ボロス): 戦闘開始時に従者の少年トークン1体を召喚', () => {
    const run = createRunInBattle(3, 'set-confirm', 'leader_boros')
    const s = run.combat!
  ...
  it('しずく (シミック): エナジー上限4で開始し、開幕1ドロー', () => {
    const run = createRunInBattle(3, 'set-confirm', 'leader_simic')
    const s = run.combat!
