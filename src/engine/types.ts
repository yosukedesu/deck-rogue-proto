...
export type GameEvent =
  ...
  | {
      ...
      /** 関与した敵 (source=playerなら対象、enemyなら攻撃者)。UIの結果演出用 (2026-09-01) */
      readonly enemyIndex?: number
...
export interface CardDef {
   ...
   * **払う量 X は 1〜現在のエナジーから選ぶ (PlayCard.xAmount。省略=全部)。上限は無い (2026-09-03 一時置いた上限4は同日撤廃=本家形: 効率を本家の比率〔1Eコモンの約80%〕に合わせ、X の大きさで制限しない)**
...
export interface CardInstance {
  ...
  /**
   * 屍集めで消滅置き場から戻した札 (2026-08-31 rework)。この戦闘中プレイ・伏せコストが0になる。
   * Xコスト札は対象外 (Xは支払った量が効果量なので0E化すると意味が壊れる)。
...
export type EnemyArchetype =
  ...
  | 'summoner' // 召喚型 (2026-09-14): 味方を産む。頭数が増える前に潰すか (苔の産み手)
  ...
  | 'guardian' // 護衛型: 生存中、仲間への単体対象を自分が受ける (盾持ちの従士 2026-09-02)
...
export interface EnemyDef {
  ...
  /**
   * 連携 (2026-09-02 陣形もの)。他の仲間が1体でも生存している間、攻撃の実値と幅表示に+N
